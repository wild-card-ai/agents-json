...
def flows_tools(flows: List[Flow], format: ToolFormat) -> List[Dict[str, Any]]:
    ...
    elif format == ToolFormat.CLAUDE:                       #Added this for Claude <-------------------------->
        return [flow_to_claude_tool(flow) for flow in flows]
    else:
...
def convert_schema_to_claude(schema: Dict[str, Any]) -> Dict[str, Any]:
